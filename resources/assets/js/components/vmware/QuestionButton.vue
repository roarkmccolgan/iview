<template>
	<div class="mb-2 p-2" :class="[question.optionLabels ? gridClass : 'bg-white rounded']">
		<h2 v-show="label" :key="'head'" class="mb-2 font-light text-xl leading-tight text-dassault-blue " :class="[question.optionLabels ? 'sm:w-1/4 sm:text-base' : 'sm:text-2xl']">{{label}}</h2>
		<transition-group
			name="fade"
			appear
			tag="div"
