<template>
	<div class="flex flex-col justify-center text-dassault-blue">
		<template v-for="(options, optKey) in question.options">
			<question-slider :theOptions="options.options" :label="options.label" v-on:selectOption="selectOption" :showDetails="showDetails" :qname="options.name"></question-slider>
		</template>
	</div>
</template>
<script>
