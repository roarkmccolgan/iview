<template>
	<div class="flex-grow">
		<div class="w-full bg-cover bg-center bg-no-repeat pt-24 pb-4" :style="`background-image: url('/images/tools/${assessment.tool.id}/homepage.jpg')`">
				...
				<div class="sm:w-1/2 px-3 mb-6">
					<div class="sm:flex bg-dassault-purple p-4">
						...
						<div class="sm:flex sm:flex-col sm:w-1/2 text-sm">
							...
							<a :href="'#'" target="_blank" class="block bg-white text-center border text-dassault-blue border-white hover:bg-white shadow py-2 px-4 rounded no-underline" @click="trackEvent('asset', 'download', 'Infobrief_'+$i18n.locale)">{{ $t('dassault.download-now') }}</a>
						</div>
					</div>
				</div>
