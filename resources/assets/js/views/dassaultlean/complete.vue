		...
		<div class="container mx-auto bg-white border-t border-b mt-6 p-8 sm:border sm:rounded shadow text-grey-darker">
			<div class="flex flex-wrap justify-center">
				<div class="w-full sm:w-1/2 relative">
					<h2 class="font-light leading-tight mb-4 text-dassault-blue">{{ $t('dassaultlean.finishtxtheading') }}</h2>
					<p>
						{{ $t('dassaultlean.finishtxt1') }}: <strong class="inline font-bold text-lg text-dassault-blue">{{ $t('dassaultlean.'+theResult.overall.rating) }}</strong>
					</p>
					<p class="mb-2">
				...
				<div class="w-full sm:w-1/2">
					<div class="sm:ml-8">
						...
						<!-- <form id="leadForm" :action="'/'+assessment.localeUrl+'pdf'" method="GET" > -->
							...
							<h2 class="flex justify-start p-2 bg-dassault-blue text-white font-light leading-tight">
								<font-awesome-icon class="mr-2" :icon="icons.faEnvelope"></font-awesome-icon>
								<div v-html="$t('dassaultlean.register')"></div>
							</h2>
							<div class="flex flex-wrap bg-white shadow-md rounded rounded-t-none px-8 py-8 pb-2 mb-4">
									...
									<div class="mr-2">
										...
										<div class="inline-block relative w-full mb-2">
											<select class="block appearance-none w-full bg-grey-lighter border text-grey-darker py-3 px-4 pr-8 rounded" :class="[errors.size ? 'border-red' : '']" id="company_size" name="extra[company_size]" @blur="hasError">
												...
												<option value="1" v-html="$t('dassaultlean.size1')"></option>
												<option value="2 to 9" v-html="$t('dassaultlean.size2')"></option>
												<option value="10 to 49" v-html="$t('dassaultlean.size3')"></option>
												<option value="50 to 99" v-html="$t('dassaultlean.size4')"></option>
												<option value="100 to 249" v-html="$t('dassaultlean.size5')"></option>
												<option value="250 to 499" v-html="$t('dassaultlean.size6')"></option>
												<option value="500 to 999" v-html="$t('dassaultlean.size7')"></option>
												<option value="1000 or more" v-html="$t('dassaultlean.size8')"></option>
											</select>
									...
									<div class="ml-2">
										...
										<div class="inline-block relative w-full mb-2">
											<select class="block appearance-none w-full bg-grey-lighter border text-grey-darker py-3 px-4 pr-8 rounded" :class="[errors.size ? 'border-red' : '']" id="industry" name="extra[industry]" @blur="hasError">
												<option value="" v-html="$t('general.pleaseselect')"></option>
												<option value="Banking/insurance/financial services" v-html="$t('dassaultlean.industry1')"></option>
												<option value="Manufacturing" v-html="$t('dassaultlean.industry2')"></option>
												<option value="Retail/wholesale" v-html="$t('dassaultlean.industry3')"></option>
												<option value="Communication" v-html="$t('dassaultlean.industry4')"></option>
												<option value="Media (TV, radio, press)" v-html="$t('dassaultlean.industry5')"></option>
												<option value="Utilities" v-html="$t('dassaultlean.industry6')"></option>
												<option value="Transportation" v-html="$t('dassaultlean.industry7')"></option>
												<option value="Construction" v-html="$t('dassaultlean.industry8')"></option>
												<option value="Personal services/leisure" v-html="$t('dassaultlean.industry9')"></option>
												<option value="Professional services (real estate, accountancy, advertising, business consultancy, recruitment, etc.)" v-html="$t('dassaultlean.industry10')"></option>
												<option value="IT/telecom service providerse" v-html="$t('dassaultlean.industry11')"></option>
												<option value="Education" v-html="$t('dassaultlean.industry12')"></option>
												<option value="Healthcare" v-html="$t('dassaultlean.industry13')"></option>
											</select>
											<div class="pointer-events-none absolute pin-y pin-r flex items-center px-2 text-grey-darker">
												<svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
											</div>
								...
								<div class="mb-6 w-full">
									<label :class="[marketing ? activeClass : normalClass, errors.marketing ? 'border-red bg-red-lightest' : '']" tabindex="0" @keyup.space="$event.target.click()" >
										...
										<div class="flex items-baseline">
											...
											<div class="flex-grow" v-html="$t('dassaultlean.marketing')">
												
											</div>
										</div>
										<p class="text-red text-xs italic" v-show="errors.marketing" v-html="$t('dassaultlean.errorMarketing')"></p>
									</label>
								</div>
								<div class="mb-6 w-full">
									<label :class="[terms ? activeClass : normalClass, errors.terms ? 'border-red bg-red-lightest' : '']" tabindex="0" @keyup.space="$event.target.click()" @click="checkTerms">
										...
										<div class="flex items-baseline">
											...
											<div class="flex-grow" v-html="$t('dassaultlean.byreg')">
								...
								<div class="mb-6 w-full">
									...
									<a href="/quiz/thankyou" class="text-center block bg-dassault-blue hover:text-ntt-gold shadow-md text-white py-4 px-6 rounded no-underline" :class="[{'cursor-not-allowed':isSubmitting}]"><font-awesome-icon class="mr-2" :class="[{'fa-spin': isSubmitting}]" :icon="isSubmitting ? icons.faSyncAlt : icons.faEnvelope"></font-awesome-icon> {{ $t('general.send') }}</a>
								</div>
							</div>
						</form>
					</div>
