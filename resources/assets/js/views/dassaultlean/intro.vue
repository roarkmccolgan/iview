<template>
	<div class="flex-grow">
		<div class="w-full h-full bg-cover bg-center bg-no-repeat pt-6 sm:py-24 pb-4" :style="`background-image: url('/images/tools/${assessment.tool.id}/homepage.jpg')`">
			<div class="container mx-auto items-center mb-4 px-4 text-dassault-blue">
				<div class="hidden sm:block flex w-full sm:w-auto mt-2 sm:mt-0" v-if="assessment.languages.length > 2000">
					<span class="inline-block mr-2 text-grey-dark text-sm"><font-awesome-icon class="ml-2" :icon="icons.faGlobe" /> {{ $t('dassaultlean.change-langauge') }}:</span>
				...
				<div class="flex flex-wrap flex-row-reverse sm:flex-no-wrap sm:flex-row items-center bg-white my-4 p-4 sm:p-10 shadow-lg">
					<div class="w-full sm:w-3/5">
						<!-- <h1 class="text-dassault-blue font-bold leading-none">{{ $t('dassaultlean.title') }}</h1> -->
						<!-- <h3 class="text-dassault-blue font-light leading-tight mb-4">{{ $t('dassaultlean.sub-title') }}</h3> -->
						<div v-html="$t('dassaultlean.landing')"></div>
					</div>
					<div class="w-full sm:w-2/5 text-center my-16 sm:m-0">
						<router-link class="sm:inline-block mx-auto bg-dassault-blue hover:text-dassault-blue-light shadow-md text-white py-4 px-6 rounded no-underline" :to="'/questions/1'">{{ $t('dassaultlean.startbut') }} <font-awesome-icon class="ml-2" :icon="icons.faArrowRight" /></router-link>
					</div>
				</div>
			</div>
		</div>
