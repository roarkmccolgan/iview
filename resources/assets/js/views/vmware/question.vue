<template>
	<div class="flex-grow bg-grey-lighter">
		<div class="px-4 flex flex-col">
			<div class="mt-2">
				<div class="container mx-auto">
					<div class="pt-2 flex items-center">
						<div class="">
							<button class="inline-block border border-grey-light hover:bg-vmware-green text-vmware-green hover:text-white pt-2 pb-1 px-2 rounded no-underline" @click.prevent="back"><font-awesome-icon :icon="faArrowLeft" /></button>
						</div>
						<div class="flex-grow hidden sm:block">
							<!-- Progress indicator -->
							<div class="flex items-center px-4">
								<template v-for="(question, qkey) in questions">
									<div class="w-5 h-5 border-2 border-vmware-green rounded-full" :class="{'bg-vmware-green': (question == currentQuestion || question.selected)}"></div>
									<div class="border-t-2 border-vmware-green flex-grow metropolislight" v-show="qkey !== ('q'+Object.keys(questions).length)"></div>
								</template>
							</div>
						</div>
			...
			<div class="container mx-auto pt-2 text-grey-darkest ">
				<span class="text-sm hidden sm:block sm:my-2">{{ $t('general.question') | toTitle}} {{currentQuestion.name}} {{$t('general.of')}} {{totalQuestions}} - {{ $t('vmware.'+currentQuestion.section+'.title') }} </span>
				<transition name="fade">
					<h1 class="font-light text-2xl sm:text-3xl leading-tight" v-show="showDetails" v-html="currentQuestion.question"></h1>
				</transition>
			</div>
			<div class="container mx-auto h-1 hrcolor my-4">
			</div>
...
import QuestionSlider from '../../components/vmware/QuestionSlider.vue';
import GroupSlider from '../../components/vmware/GroupSlider.vue';
import GroupButton from '../../components/vmware/GroupButton.vue';
import GroupOpposingSlider from '../../components/vmware/GroupOpposingSlider.vue';
...
export default{
	data () {
		return {
			...
			answer: [],
			saving: false,
			normalButton: 'block w-full py-4 px-4 text-center bg-vmware-green text-white',
			disabledButton: 'block w-full py-4 px-4 text-center bg-vmware-green text-white opacity-50 cursor-not-allowed',
			error: false,
			errorClass: 'border-red-lighter bg-red-lightest text-red-light',
			noticeClass: 'border-blue-light bg-blue-lighter text-vmware-green',
			result: [],
		}
	},
	computed: {
