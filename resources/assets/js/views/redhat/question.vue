<template>
	<div class="flex-grow">
		...
		<div class="flex flex-col" v-else>
			...
			<div class="" v-else>
				<div class="bg-blackblack pt-4 sm:pt-8">
					<div class="container mx-auto px-4 sm:px-0">
						<div class="pt-2 flex items-center">
							<div class="">
								<button class="inline-block border border-grey-light hover:bg-redhat text-white hover:text-white pt-2 pb-1 px-2 rounded no-underline" @click.prevent="back"><font-awesome-icon :icon="faArrowLeft" /></button>
							</div>
							<div class="flex-grow hidden sm:block">
								<!-- Progress indicator -->
								<div class="flex items-center px-4">
									<template v-for="(question, qkey) in questions">
										<div class="w-4 h-4 border-2 border-white rounded-full" :class="{'bg-redhat': (question == currentQuestion || question.selected)}"></div>
								...
								<div class="flex items-center justify-between px-4 my-2 mt-4">
									<template v-for="(section, secKey) in questionsBySection">
										<div class="text-center rounded h-1 overflow-hidden" :class="[section.section == currentQuestion.section ? `bg-redhat` : 'bg-grey-light',secKey == 0 ? 'cmr' : (secKey == questionsBySection.length - 1 ? 'cml' : 'cmx')]" :style="`width: ${secKey == 0 || secKey == questionsBySection.length - 1 ? (section.total*8.333333333) - 4.166666666 : section.total*8.333333333}%`">
										</div>
									</template>
								</div>
							</div>
						...
						<div class="py-4 sm:py-8 text-white flex items-center">
							<h2 class="font-black sm:mt-0 text-redhat text-xl sm:text-3xl">{{ currentQuestion.title }}</h2>
							<v-popover
								offset="16"
								placement="right"
								>
				...
				<div class="container mx-auto py-2 flex-grow px-4 sm:px-0">
					...
					<transition name="fade">
						<div>
							<h1 class="font-bold text-redhat-blue-light text-xl sm:text-3xl leading-tight mb-4" v-show="showDetails" v-html="currentQuestion.question"></h1>
					...
					<template v-else-if="currentQuestion.type == 'slider'">
						<question-slider :key="currentQuestion.qKey" :question="currentQuestion" :the-options="currentQuestion.options" v-on:selectOption="selectOption" :showDetails="showDetails" :qname="currentQuestion.qKey" :answer="answer"></question-slider>
					</template>
					...
					<template v-else-if="currentQuestion.type == 'opposingslider'">
						<opposing-slider :answer="answer" :question="currentQuestion" :the-options="currentQuestion.options" @selectOption="selectOption" :showDetails="showDetails"></opposing-slider>
					</template>
...
//Question components
import QuestionButton from '../../components/redhat/QuestionButton.vue';
import QuestionSlider from '../../components/redhat/QuestionSlider.vue';
import GroupSlider from '../../components/redhat/GroupSlider.vue';
import GroupButton from '../../components/redhat/GroupButton.vue';
import OpposingSlider from '../../components/redhat/OpposingSlider.vue';
import GroupOpposingSlider from '../../components/redhat/GroupOpposingSlider.vue';

//import NuggetComponent from '../../components/redhat/NuggetComponent.vue';
import IntermissionComponent from '../../components/redhat/IntermissionComponent.vue';
import IntroComponent from '../../components/redhat/IntroComponent.vue';
...
export default{
	data () {
		return {
			...
			answer: [],
			saving: false,
			normalButton: 'block w-full py-4 px-4 text-center bg-redhat text-white',
			disabledButton: 'block w-full py-4 px-4 text-center bg-redhat text-white opacity-50 cursor-not-allowed',
			error: false,
			errorClass: 'border-red-lighter bg-red-lightest text-red-light',
			noticeClass: 'border-blue-light bg-blue-lighter text-redhat-blue',
			result: [],
		}
	},
	computed: {
					...
					if(that.questions.hasOwnProperty('q'+nextQ)){
						...
						that.getResults().then(function (response) {
							if(response.data.query == 'success'){
								console.log('save_successful now what');
