<template>
	<div class="flex-grow">
		...
		<div class="flex flex-col" v-else>
			...
			<div class="" v-else>
				<div class="bg-snow-light pt-4 sm:pt-8 text-snow-dark">
					<div class="container mx-auto px-4 sm:px-0">
						<div class="pt-2 flex items-center">
							<div class="">
								<button class="inline-block border border-grey-light hover:bg-snow-blue text-white hover:text-white pt-2 pb-1 px-2 rounded no-underline" @click.prevent="back"><font-awesome-icon :icon="faArrowLeft" /></button>
							</div>
							<div class="flex-grow hidden sm:block">
								<!-- Progress indicator -->
								<div class="flex items-center px-4">
									<template v-for="(question, qkey) in questions">
										<div class="w-4 h-4 border-2 border-white rounded-full" :class="{'bg-snow-dark': (question == currentQuestion || question.selected)}"></div>
								...
								<div class="flex items-center justify-between px-4 my-2 mt-4">
									<template v-for="(section, secKey) in questionsBySection">
										<div class="mx-1 text-center rounded h-1 overflow-hidden" :class="[section.section == currentQuestion.section ? `bg-snow-dark` : 'bg-grey-light',secKey == 0 ? 'cmr' : (secKey == questionsBySection.length - 1 ? 'cml' : 'cmx')]" :style="`width: ${secKey == 0 || secKey == questionsBySection.length - 1 ? (section.total*7.142857143) - 3.571428572 : section.total*7.142857143}%`">
										</div>
									</template>
								</div>
							</div>
						...
						<div class="py-4 sm:py-8 text-white flex items-center">
							<h2 class="font-snow sm:mt-0 text-snow-dark text-xl sm:text-3xl">{{ currentQuestion.title }}</h2>
							<!-- <v-popover
								offset="16"
								placement="right"
								>
				...
				<div class="container mx-auto py-2 flex-grow px-4 sm:px-0">
					...
					<transition name="fade">
						<div>
							<h1 class="font-bold text-snow-dark-light text-xl sm:text-3xl leading-tight mb-4" v-show="showDetails" v-html="currentQuestion.question"></h1>
...
import QuestionSlider from '../../components/snow/QuestionSlider.vue';
import GroupSlider from '../../components/snow/GroupSlider.vue';
import GroupButton from '../../components/snow/GroupButton.vue';
import GroupOpposingSlider from '../../components/snow/GroupOpposingSlider.vue';

//import NuggetComponent from '../../components/snow/NuggetComponent.vue';
import IntermissionComponent from '../../components/snow/IntermissionComponent.vue';
import IntroComponent from '../../components/snow/IntroComponent.vue';
...
export default{
	data () {
		return {
			...
			answer: [],
			saving: false,
			normalButton: 'block w-full py-4 px-4 text-center bg-snow-dark text-white',
			disabledButton: 'block w-full py-4 px-4 text-center bg-snow-dark text-white opacity-50 cursor-not-allowed',
			error: false,
			...
			if (this.questions.hasOwnProperty(q)){
				if(!this.questions[q].selected){
					...
					if(this.questions[q].name !== this.currentQuestion.name){
						let path = that.$i18n.locale == 'en' ? `/questions/${this.questions[q].name}` : `/${that.$i18n.locale}/questions/${this.questions[q].name}`;
