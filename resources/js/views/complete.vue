									...
									<div class="ml-2">
										...
										<div class="inline-block relative w-full mb-2">
											<select class="block appearance-none w-full bg-grey-lighter border text-grey-darker py-3 px-4 pr-8 rounded" :class="[errors.turnover ? 'border-red' : '']" id="turnover" name="extra[turnover]" @blur="hasError">
												<option value="" v-html="$t('general.pleaseselect')"></option>
												<option value="Less than €100 million" v-html="$t('general.less-than-100m')"></option>
												<option value="€100-250 million" v-html="$t('general.100m-250m')"></option>
												<option value="€250 million - €1 billion" v-html="$t('general.250m-1b')"></option>
												<option value="More than €1 billion" v-html="$t('general.more-than-1b')"></option>
											</select>
											<div class="pointer-events-none absolute pin-y pin-r flex items-center px-2 text-grey-darker">
												<svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
											</div>
