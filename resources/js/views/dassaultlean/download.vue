...
export default{
	...
	computed: {
		transText: function(){
			if(!this.ready){
				return {
					heading: this.$t('dassaultlean.report.generating'),
					sentence: ''
				};
			} else if(this.ready && !this.expired){
				return {
					heading: this.$t('dassaultlean.report.ready'),
					sentence: this.$t('dassaultlean.report.readySentence', { url: this.downloadUrl }),
				};
			} else if(this.ready && this.expired){
				return {
					heading: this.$t('dassaultlean.report.regenerate'),
					sentence: this.$t('dassaultlean.report.regenerateSentence'),
				};
			}
		}
	},
