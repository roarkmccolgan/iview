...
export default{
	...
	computed: {
		transText: function(){
			if(!this.ready){
				return {
					heading: this.$t('redhat.report.generating'),
					sentence: ''
				};
			} else if(this.ready && !this.expired){
				return {
					heading: this.$t('redhat.report.ready'),
					sentence: this.$t('redhat.report.readySentence', { url: this.downloadUrl }),
				};
			} else if(this.ready && this.expired){
				return {
					heading: this.$t('redhat.report.regenerate'),
					sentence: this.$t('redhat.report.regenerateSentence'),
				};
			}
		}
	},
