<?php

//sage
return [
    'title' => 'Saca el m&aacute;ximo partido a la nube',
    'sub-title' => 'Avanzando en su desarrollo',
    'stage1' => 'Fase 1: Momento de actuar',
    'stage2' => 'Fase 2: Momento de comprometerse',
    'stage3' => 'Fase 3: Momento de invertir',
    'stage4' => 'Fase 4: Momento de mejorar ',
    'stage5' => 'Fase 5: Momento de acelerar',
    'introduction' => '
    <p>
        Gracias por utilizar la herramienta de evaluación del nivel de adopción de la nube de IDC (Cloud Benchmark Assessment). Este estudio te permite posicionar a tu empresa en comparación con más de 500 organizaciones similares en Estados Unidos y Europa, y te ofrece una guía esencial para el desarrollo de tu negocio en la nube. 
    </p>
    <p>
        En el siguiente informe se proporciona información sobre la situación de tu organizaciónen comparación con otras empresas similares del mercado, y una serie de recomendaciones sobre cómo dirigir tu negocio hacia un futuro rentable y de éxito en la nube.
    </p>
    ',
    'complete_thankyou' => 'Gracias :Fname,',
    'complete_body' => 'Tu informe personalizado llegará pronto a tu bandeja de entrada',
    'complete_tweet' => 'I\'ve just taken the Cloud Development Assessment Sponsored by @Sage %26 @IDCUKI, we\'re at :result. Interested?', // http://bit.ly/2g8WN1d
    'email'=>[
        'subject' => 'Informe de IDC',
        'hi' => 'Hola :Fname',
        'body' => '
            Aquí está tu informe personalizado acerca del nivel de madurez del negocio en la nube .<br/>
            El informe desglosa los resultados por categorías y te permite comparar tu empresa con otras organizaciones.
        ',
        'link' => 'Descargar el informe',
        'extra' => 'Una vez que hayas leído el informe, ¿por qué no visitas <a href="http://www.sage.es/" style="color:#024886">Sage</a>?',
        'regards' => 'Atentamente',
        'team' => 'El equipo de IDC',
    ],
    'introduction-image' => 'report1.jpg',
    'overall' => [
        'title'=>'Adopción general de la nube',
        'image'=>'',
        'introduction' => '
        <p>
            Según la evaluaciónde IDC, tu organización se encuentra en la fase <strong>:result</strong> en lo que respecta a tu nivel de adopción general de la nube. El <strong>:benchmark</strong>% de las organizaciones que ha evaluado IDC se encuentran en esta fase. La mayoría de las organizaciones están en la fase 3: Momento de invertir.
        </p>
        <p>
            Para que dispongas de una visión más completa con recomendaciones precisas, el informe muestra los componentes de la estrategia en la nube, y te ofrece una evaluación de tu nivel de desarrollo en cada una de ellas, así como recomendaciones específicas sobre cómo mejorarlas.
        </p>
        ',
        'stage1'=>'
            <p>
                En esta fase de adopción de la nube, las organizaciones tienden a proporcionar la nube a petición de sus clientes o por ser distribuidores de un proveedor. En consecuencia, los ingresos proceden de la prescripción o de la distribución. Las organizaciones, por lo general, están centradas en los ingresos de proyectos concretos en vez de en generar flujos de ingresos recurrentes a más largo plazo. Todavía tienen que encaminarse hacia un futuro en la nube y, como resultado, deben esforzarse en generar ingresos en esa área. No tienen definida una estrategia para la nube, pero es el momento de que los responsables de la toma de decisiones actúen para que se lleve ya a cabo la transición.
            </p>
            <p>
                En el siguiente informe se proporciona una explicación más detallada de las diferentes áreas de tu estrategia para la nube y de tu progreso. También se ofrece orientación para mejorar dichas áreas de tu negocio. 
            </p>
        ',
        'stage2'=>'
            <p>
                En esta fase de adopción de la nube, las organizaciones empiezan a desarrollar y a proporcionar modelos de negocio específicos en la nube. En general, no son lo suficientemente detallados como para tener en cuenta las distintas líneas de productos o su contribución en la facturación, y no suelen estar gestionados ni tener un objetivo específico. Pocos indicadores y métricas, la casi ausencia de automatización de los procesos, falta de metodología y la venta o prestación de servicios es escasa. Aunque en esta fase las empresas empiezan a consolidar las funciones comerciales y de marketing en la nube, los ingresos siguen procediendo de la venta de soluciones on-premise. A pesar de ello, los ingresos recurrentes superan a los del modelo de negocio tradicional.
            </p>
            <p>
                En el siguiente informe se proporciona una explicación más detallada de las diferentes áreas de tu estrategia para la nube y de tu progreso. También se ofrece orientación para mejorar dichas áreas de tu negocio.
            </p>
        ',
        'stage3'=>'
            <p>
                En esta fase, las organizaciones disponen de un sólido plan de negocio en la nube y están desarrollando estrategias específicas para los productos, que incluye los servicios profesionales. Han invertido en la nube y, lo que es más importante, tienen planes de inversión para los dos o tres próximos años. Asimismo, están empezando a supervisar y a establecer objetivos distintos para los ingresos basados en la nube, de forma independiente de los ingresos por licencias o de otro tipo. Muchas empresas también están creando procesos que sean reutilizables y están empezando a automatizar las funciones clave. Las estrategias de marketing muestran signos de perfeccionamiento, aunque muchas organizaciones aún necesitan abandonar los planteamientos de marketing tradicional. En esta fase, es probable que las organizaciones hayan establecido un roadmap bien estructurado sobre tecnología y que hayan empezado a invertir en infraestructura, herramientas y tecnología de gestión para la nube.
            </p>
            <p>
                En el siguiente informe se proporciona una explicación más detallada de las diferentes áreas de tu estrategia para la nube y de tu progreso. También se ofrece orientación para mejorar dichas áreas de tu negocio.
            </p>
        ',
        'stage4'=>'
            <p>
                En esta fase de adopción de la nube, las organizaciones han desarrollado una planificación exhaustiva en diferentes productos, soluciones y arquitecturas para la nube. Esto incluye las estrategias de mercado y el conjunto de tareas para mejorar la ejecución de actividades de venta. Muchas organizaciones han abierto un abanico de ofertas en la nube y de servicios relacionados, principalmente dedicados a las ventas, implementaciones y funciones de soporte específicas para la nube. Normalmente, llegados a esta fase, las organizaciones pueden ejecutar varias arquitecturas en la nube y han identificado las áreas de especialización, lo que también les permite medir y evaluar los ingresos recurrentes. La mayoría ha establecido herramientas y metodologías para conseguir implementaciones más eficientes. Con ello se garantiza las renovaciones y la optimización de las soluciones.
            </p>
            <p>
                En el siguiente informe se proporciona una explicación más detallada de las diferentes áreas de tu estrategia para la nube y de tu progreso. También se ofrece orientación para mejorar dichas áreas de tu negocio.
            </p>
        ',
        'stage5'=>'
            <p>
                Para las organizaciones que se encuentran en esta fase, la nube determina la inversión y la toma de decisiones relativas a la futura orientación de sus negocios. También están firmemente decididas a apostar por un modelo de negocio de ingresos recurrentes. Todos los aspectos del negocio se centran en un amplio conjunto de KPIs, desde los objetivos de captación de clientes y de crecimiento hasta los niveles de satisfacción y los porcentajes de renovación. Todos los sistemas y procesos están diseñados para optimizar la rentabilidad de la nube.Muchas organizaciones tienen procesos en la nube totalmente automatizados para ofrecer economías de escala. Esencialmente, se hace hincapié en la venta de soluciones, así como en el desarrollo y la optimización de la propiedad intelectual interna.
            </p>
            <p>
                En el siguiente informe se proporciona una explicación más detallada de las diferentes áreas de tu estrategia para la nube y de tu progreso. También se ofrece orientación para mejorar dichas áreas de tu negocio.
            </p>
        ',
    ],
    'cloud-vision-and-strategy' => [
        'title'=>'Estrategia y plan para la nube',
        'pageimage'=>'',
        'stage1'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 1 en lo que respecta a tu estrategia y plan para la nube. El 14 % de las organizaciones que han participado en nuestroestudio se encuentran en esta fase. La mayoría de ellas están en la fase 2. De forma general, en esta fase de desarrollo de su estrategia, las organizaciones comparten las siguientes características:
        </p>
        <h3>Planificación:</h3>
        <p>
            No tienen definido un plan para un negocio futuro en la nube o cualquier tipo de estrategia global. Su nivel de adopción de la nube es más bien ad hoc y responde a una necesidad urgente. Todavía tienen que adoptar un futuro completo en la nube y, en consecuencia, deberán esforzarse para generar ingresos en esa área.
        </p>
        <h3>Ejecución: </h3>
        <p>
            La mayoría de las organizaciones no están orientadas a funcionar como una empresa en la nube. Por ello, dirigirán sus solicitudes de la nube a sus Partners proveedores o distribuirán servicios en la nube pública a una base reducida de clientes ya existentes. Más importante aún es que, sin objetivos de ingresos específicos basados en la nube, muchas organizaciones carecen de criterios para medir su éxito en la nube y de cualquier orientación para promover el desarrollo de esta parte de sus negocios.
        </p>
        <img src="http://sage.idcready.net/images/tools/1/cloud-vision-and-strategy.jpg" class="right">
        <h3>Recomendaciones</h3>
        <h5>Apoyo de la dirección de la empresa:</h5>
        <p>
            Este factor es fundamental si vas a desarrollar tu negocio en la nube. Los directivos de la empresa deben iniciar el proceso de definición de una estrategia y de consolidación de un equipo de trabajo para crear e implementar un modelo de negocio y un plan de inversiones para la nube.
        </p>
        <h5>Evaluación de las oportunidades:</h5>
        <p>
            Para contar con el apoyo suficiente de la dirección de la empresa en las distintas líneas de negocio, te recomendamos comenzar por una evaluación de las oportunidades. Antes de nada, analiza el interés por la nube entre los sectores clave con los que trabajas.
        </p>
        <h5>Atender las inquietudes: </h5>
        <p>
            Es muy posible que te topes con cierto grado de temor, incertidumbre y duda al comunicar la planificación preliminar. Te recomendamos empezar por atender directamente las inquietudes, antes de desarrollar la estrategia y el plan de negocio.
        </p>
        ',
        'stage2'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 2 en lo que respecta a tu estrategia y plan para la nube. El 39 % de las organizaciones que han participadoen nuestro estudio se encuentran en esta fase, y la mayoría de ellas también están en la fase 2. De forma general, en esta fase de desarrollo de su estrategia y plan, las organizaciones comparten las siguientes características:
        </p>
        <h3>Planificación:</h3>
        <p>
            Las organizaciones comienzan a desarrollar modelos de negocio y programas orientados a la nube. Sin embargo, por lo general, no son lo suficientemente detallados como para tener en cuenta las distintas líneas de productos (por ejemplo:volumen frente a valor de Saas). Aunque la estrategia de ventas para la nube y de captación de clientes está bien definida, las organizaciones tienden a centrarse solo en los mercados existentes. Puede queconozcanbien los sectores y los mercados en los que desean especializarse, pero es poco probable que se haya definido una estrategia de venta.
        </p>
        <h3>Ejecución: </h3>
        <p>
            Aunque se están llevando a la práctica algunas iniciativas orientadas a la nube, las inversiones en esa área suelen ser reducidas. Dichas iniciativas tienden a ser pequeños reductos de actividad estratégica más que elementos de una estrategia de ejecución más cohesionada. En las ventas, se está empezando a ejecutar una estrategia de venta en la nube, respaldada por objetivos de ingresos y el desarrollo de un plan de negocio.
        </p>
        <img src="http://sage.idcready.net/images/tools/1/cloud-vision-and-strategy.jpg" class="right">
        <h3>Recomendaciones</h3>
        <h5>Estrategias definidas por producto:</h5>
        <p>
            A medida que tu negocio en la nube gane impulso, te recomendamos estructurar tu estrategia de planificaciónsegún las diferentes líneas de producto. Esto resulta esencial para los negocios de SaaS, ya sean de volumeno de valor, en los que las características y los requisitos de lanzamiento al mercado varían considerablemente.
        </p>
        <h5>Implicación del equipo directivo:</h5>
        <p>
            Céntrate en garantizar el respaldo y la implicación del equipo directivoen tu estrategia para la nube. Es necesario que dicho respaldo provenga de todas las funciones empresariales clave (altos directivos, marketing, ventas, operaciones y finanzas) para adaptarse e invertir de forma adecuada a medida que el negocio de la nube comience a desarrollarse.
        </p>
        <h5>Crea modelos financieros:</h5>
        <p>
            en tu planificación para la nube debes tener en cuenta el crecimiento, los ingresos y, especialmente, la rentabilidad. Asesórate y crea modelos financieros en los que se analice el posible impacto de los ingresos recurrentes en los ingresos brutos y la rentabilidad.
        </p>
        ',
        'stage3'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 3 en lo que respecta a tu estrategia y plan la nube. El 28 % de las organizaciones que han participado en nuestro estudio se encuentran tambiénen esta fase. La mayoría de ellas están en la fase 3. De forma general, en esta fase de desarrollo de su estrategia y plan para la nube, las organizaciones comparten las siguientes características:
        </p>
        <h3>Planificación:</h3>
        <p>
            Las organizaciones cuentan con un plan de negocio para la nube y están desarrollando estrategias específicas por línea de productos (por ejemplo, Saas por volumen o por valor). Asimismo, planifican incorporar propuestas de servicios profesionales más amplias, como consultoría, integración de sistemas, migración y servicios gestionados.
        </p>
        <h3>Ejecución: </h3>
        <p>
            En las líneas básicas de negocio existe el respaldo e implicación del equipo ejecutivo respecto a la nube. Por consiguiente, la organización está haciendo realidad de forma proactiva las oportunidades de la nube, realizando inversiones como parte de una estrategia empresarial a más largo plazo para los próximos dos o tres años. La organización también está buscando nuevos objetivos fuera de su base de clientes actuales(nuevos mercados, nuevos segmentos, nuevos territorios) para ampliar su negocio.
        </p>
        <img src="http://sage.idcready.net/images/tools/1/cloud-vision-and-strategy.jpg" class="right">
        <h3>Recomendaciones</h3>
        <h5>Feedback:</h5>
        <p>
            Las empresas de éxito suelen usar la nube internamente para sus propias operaciones y aprovechan esas experiencias no solo para familiarizar a toda la organización con la nube, sino también para poder desarrollar prácticas recomendadas.
        </p>
        <h5>Plan de rentabilidad definido:</h5>
        <p>
            Tu planificación financiera debe incluir un plan de rentabilidad a tres años, bien definido, que tenga en cuenta varias situaciones en relación con el impacto de los ingresos recurrentes en los ingresos brutos y la rentabilidad.
        </p>
        <h5>Trabajar estrechamente con los proveedores:</h5>
        <p>
            Comunica parte de tu estrategia y plan para la nube a tus proveedores estratégicos, y empieza a desarrollar procesos para llevar a cabo una planificación conjunta, así como revisiones de rendimiento periódicas. En esta fase, que respalden y aprueben tu estrategia podría ayudarte considerablemente en tu experiencia de la nube.
        </p>
        ',
        'stage4'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 4 en lo que respecta a tu estrategia y plan para la nube. El 13 % de las organizaciones que han participado en nuestro estudio se encuentra en esta fase. La mayoría de ellas están en la fase 2. Por lo general, en esta fase de desarrollo de su estrategia y plan para la nube, las organizaciones comparten las siguientes características:
        </p>
        <h3>Planificación:</h3>
        <p>
            Las organizaciones disponen de una planificación exhaustiva en diferentes productos, soluciones y arquitectura en la nube, así como de estrategias de mercado. Asimismo, la planificación se centra en una mayor especialización y diferenciación a través de soluciones específicas. También están definidos los planes asociados de inversión y de orientación al mercado.
        </p>
        <h3>Ejecución: </h3>
        <p>
            Las organizaciones están ejecutando su estrategia y plan para la nube, y han abierto un abanico de ofertas en la nube y de servicios relacionados mediante actividades específicas de venta, implementación y soporte. También pueden operar en varias arquitecturas en la nube y han invertido lo suficiente para llevar a cabo su estrategia. Además, establecen de forma muy activa estrategias de crecimiento y planes de mercado con el objetivo de captarnuevos clientes y expandirse geográficamente.
        </p>
        <img src="http://sage.idcready.net/images/tools/1/cloud-vision-and-strategy.jpg" class="right">
        <h3>Recomendaciones</h3>
        <h5>Adoptar un enfoque vertical:</h5>
        <p>
            Adopta un enfoque vertical para tus estrategias de ejecución y obtener de esta forma ventajas; es decir, lleva a cabo estrategias de mercado que se dirijan a sectores concretos. Ello implicaunir las soluciones, los recursos comerciales y el material de marketing para permitir dicho enfoque.
        </p>
        <h5>Empezar a desarrollar la propiedad intelectual:</h5>
        <p>
            Como parte de tu proceso de planificación estratégica, es recomendable que elabores una estrategia técnica que se centre en el desarrollo de soluciones y de arquitecturas de referencia con la posibilidad de que se reutilicen e, incluso, se comercialicen como productos independientes.
        </p>
        <h5>Estrategia de transformación digital:</h5>
        <p>
            Tu estrategia y plan también deben abarcar la elaboración de una estrategia de transformación digital más amplia, que incluya tus actuales estrategias para la nube.
        </p>
        ',
        'stage5'=>'
        <p>
            Según la evaluaciónde IDC, tu organización se encuentra en la fase 5 respecto a tu estrategia y plan para la nube. Solo el 4 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría de ellas están en la fase 2. Por consiguiente, tuorganización se encuentra por delante de otras empresas similares a la tuya en el mercado, y se caracteriza por ser «un negocio en la nube», en vez de que «la nube forme parte de tu oferta». De forma general, en esta fase de desarrollo de su estrategia y plan para la nube, las organizaciones comparten las siguientes características:
        </p>
        <h3>Planificación:</h3>
        <p>
            Las inversiones y decisiones importantes sobre el futuro de la empresa se basan en la nube.Asimismo, las organizaciones apuestan por un modelo de negocio basado en ingresos recurrentes, lo que impulsa su crecimiento y especialización. Han elaborado planes específicos para dirigirse a nuevos mercados y sectores, y trabajan estrechamente con sus principales proveedores para ofrecer una planificación común de clientes y de marketing.
        </p>
        <h3>Ejecución: </h3>
        <p>
            Todos los aspectos del negocio se centran en un amplio conjunto de KPIs, desde los objetivos de captación de clientes y de crecimiento hasta la evaluación de los niveles de satisfacción. Además, los servicios profesionales desempeñan un papel clavetanto en los objetivos empresariales como en la entrega de soluciones en la nube. Lasorganizaciones que se encuentran en esta fase también están en condiciones de ofrecer cualquier tipo de arquitectura en la nube a sus clientes. La escalabilidad es un factor clave, por lo que llevan a cabo estrategias de crecimiento y escalabilidad, normalmente orientadas a nuevos clientes y mercados. 
        </p>
        <img src="http://sage.idcready.net/images/tools/1/cloud-vision-and-strategy.jpg" class="right">
        <h3>Recomendaciones</h3>
        <h5>Mirar al futuro:</h5>
        <p>
            Mira siempre hacia el futuro para tener presente cómo evolucionarán las soluciones en la nubey ampliar la integración de los procesos y tecnologías de transformación digital. También debes analizar la expansión geográfica y sectorial de tu negocio y tus propuestas. 
        </p>
        <h5>Centrarse en el desarrollo de la propiedad intelectual:</h5>
        <p>
            Es importante que definas un plan estratégico para que tu organización se centre en el desarrollo, la actualización y la venta de propiedad intelectual a tu base de clientes, tantoactuales comofuturos. Este planteamiento te ayudará a definir tus estrategias de ejecución y planificación futuras.
        </p>
        <h5>Diferenciarse mediante soluciones: </h5>
        <p>
            Proporciona propuestas de valor a los clientes ofreciendo soluciones que resuelven la problemáticade negocio. Todas las actividades de planificación estratégica deben centrarse en solucionar problemas de negocio a tus clientes.
        </p>
        ',
    ],
    'process-and-skill-sets' => [
        'title'=>'Procesos y conocimientos técnicos',
        'pageimage'=>'process-and-skill-sets.jpg',
        'stage1'=>'
        <p>
            Según la evaluaciónde IDC, tu organización se encuentra en la fase 1 en lo que respecta al desarrollo de tus procesos en la nube. El 25 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría de ellas están en la fase 2. Por lo general, en esta fase de su desarrollo, las organizaciones comparten las siguientes características:
        </p>
        <h3>Medición:</h3>
        <p>
            Las organizaciones que se encuentran en esta fase, tienden a recurrir a la nube por una necesidad urgente, y tienen definidos pocos indicadores. Por consiguiente, las ofertas de la nube no están gestionadas y no tienen objetivos. 
        </p>
        <h3>Automatización:</h3>
        <p>
            Muchos proyectos de la nube se entregan ad hoc y hay muy pocos procesos o metodologías que se formalicen o se reutilicen. La automatización limitada de los procesos de venta y procesos de entrega ejerce una presión mayor en los márgenes.
        </p>
        <h3>Recomendaciones</h3>
        <h5>La medición es fundamental: </h5>
        <p>
            Debes empezar a analizar el crecimiento de los ingresos recurrentes separando los ingresos del negocio de la nube de los ingresos existentes basados en la venta de licencias. Además, la prestación de servicios en la nube y el impacto de los ingresos recurrentes en los ingresos brutos y netos como procesos a largo plazo.
        </p>
        <h5>Automatización:</h5>
        <p>
            Como tu negocio en la nube todavía es pequeño, no se aprecia el impacto de una escasa automatización. Sin embargo, a medida que empiece a aumentar, tu organización necesitará metodologías reutilizables y prestación de servicios automatizados para garantizar la rentabilidad. Empieza probando y eligiendo herramientas de automatización mientras tu volumen de negocio sea lo suficientemente pequeño como para gestionarlo sin ellas. De esta manera, puedes realizar una planificación de futuro.
        </p>
        <h5>Evaluación de la falta de personal especializado:</h5>
        <p>
            Según tu necesidad de personal especializado a corto, medio y largo plazo, debes saber qué te falta y dónde invertir. Empieza por elaborar un plan de formación por departamentos: por ejemplo, área técnica, marketing y ventas.
        </p>
        ',
        'stage2'=>'
        <p>
            Según la evaluaciónde IDC, tu organización se encuentra en la fase 2 respecto al desarrollo de tus procesos en la nube. El 33 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría está en la fase 2. Por consiguiente, es posible que eldesarrollo de tus procesos necesite mejorar y quede trabajo por hacer respecto a la implementación y prestación de servicios en la nube. En general, las organizaciones que se encuentran en esta fase comparten las siguientes características:
        </p>
        <h3>Medición:</h3>
        <p>
            En esta fase, las organizaciones aún tienen que tratar la nube de forma independiente en su sistema financiero. Como resultado, hay medición limitadaacerca del éxito en la nube. A pesar de ello, han desarrollado y puesto en práctica objetivos de crecimiento.
        </p>
        <h3>Automatización:</h3>
        <p>
            Normalmente, hay interés por crear procesos y metodologías repetibles para implementar los proyectos de la nube. Ello implica reconocer que la automatización es esencial para gestionar un negocio rentable en la nube. Sin embargo, esas funciones suelen estar en sus primeras fases de desarrollo.
        </p>
        <h3>Personal especializado:</h3>
        <p>
            En esta fase, se anima a que el personal asista a cursos de formación específica del proveedor, aunque dicha formación aún es bastante ad hoc, suele ser autodidacta y no se integra en un plan de formación determinado.
        </p>
        <h3>Recomendaciones</h3>
        <h5>Crear KPIs:</h5>
        <p>
            Comienza a implementar KPIs y métricas específicaspara la nube tanto en las ventas como en las operaciones, que incluyan los objetivos de crecimiento para la nube. De este modo, podrás supervisar los objetivos establecidos, y trabajar para alcanzarlos, en relación con la contribución de los ingresos recurrentes como un porcentaje de los ingresos totales. 
        </p>
        <h5>Creación de procesos reutilizables:</h5>
        <p>
            Debe convertirse en un objetivo a largo plazo, con sistemas, metodologías y KPIs diseñados para optimizar la rentabilidad de la nube mediante ingresos recurrentes de margen elevado.
        </p>
        <h5>Certificación y formación:</h5>
        <p>
            A medida que avancesen tu nivel de adopción de la nube, tendrás que organizar un plan estructurado de certificación y formación con objetivos definidos. Esto te ayudará a adquirir las competencias necesarias para que tu negocio en la nube prospere y alcanzar niveles de colaboración más altos con tus Partners proveedores. 
        </p>
        ',
        'stage3'=>'
        <p>
            Según la evaluaciónde IDC, tu organización se encuentra en la fase 3 en lo que respecta al desarrollo de tus procesos y conocimientos técnicos. El 17 % de las organizaciones que han participado en nuestro estudiose encuentran en esta fase. La mayoría de ellas están en la fase 2. En general, en esta fase de su desarrollo, las organizaciones comparten las siguientes características:
        </p>
        <h3>Medición:</h3>
        <p>
            Las organizaciones han desarrollado objetivos e indicadores específicos para medir los resultados de los servicios en la nube. Muchas han empezado a supervisar y centrarse enlos ingresos basados en la nube de forma independiente a los ingresos por licencias y de otro tipo. 
        </p>
        <h3>Automatización:</h3>
        <p>
            En esta fase, la mayoría de las empresas está creando procesos reutilizables para las implementaciones de la nube y está automatizando muchas de sus funciones clave. Habitualmente, no disponen todavía de una supervisión exhaustiva del uso y la optimización.
        </p>
        <h3>Personal especializado:</h3>
        <p>
            Llegados a este punto, las empresas están más organizadas en torno a la adquisición de personal especializado en la nube. Han comenzado a crear planes estructurados de certificación y formación con objetivos definidos. También han creado KPIs en las funciones empresariales básicas (área técnica, ventas y marketing) que se miden de forma activa.
        </p>
        <h3>Recomendaciones</h3>
        <h5>Automation and Medición:</h5>
        <p>
            Debes medir los resultados en toda la organización y, externamente, en el cliente. Esto implica conocer el modo en que los clientes utilizan, optimizan y evalúan las soluciones en la nube. De este modo, esas tendencias y pautas se pueden repetir en otros clientes.
        </p>
        <h5>Procesos y herramientas reutilizables:</h5>
        <p>
            A medida que vayas avanzando, debes crear, probar, implementar y mejorar procesos y herramientas reutilizables, entre los que se incluyen arquitecturas y códigos que reducirán los esfuerzos de implementación. Con el tiempo, conseguirás un crecimiento sostenible, economías de escala y un negocio en la nube más rentable y eficiente.
        </p>
        <h5>Herramientas analíticas:</h5>
        <p>
            En esta fase también es recomendable que empieces a seleccionar y probar herramientas analíticas para las áreas de la empresa básicas (ventas, operaciones de entrega y marketing). Dichas herramientas te permitirán diferenciarte a medida que vayasdesarrollando tu negocio en la nube.
        </p>
        ',
        'stage4'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 4 y en los primeros puestos de la clasificación general en lo que respecta al desarrollo de tus procesosy competencias en la nube. El 17 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría está en la fase 2. De forma general, en esta fase de su desarrollo, las organizaciones comparten parte de las siguientes características:
        </p>
        <h3>Medición:</h3>
        <p>
            Las organizaciones han desarrollado y perfeccionado indicadores y métricas para medir el resultado de sus modelos de negocio basados en ingresos recurrentes. Algunas empresas están empezando a medir el valor del ciclo completo de los clientes, incluidos los KPIs sobre la retención de clientes, los clientes ganados y perdidos y, en algunos casos, la satisfacción del cliente.
        </p>
        <h3>Automatización:</h3>
        <p>
            La mayoría ha establecido herramientas y metodologías para garantizar implementaciones eficientes, las renovaciones de los contratos de servicios por parte de los clientes y la optimización de soluciones. Los desarrollos continuos de las funciones de automatización también permiten que el negocio se amplíe.
        </p>
        <h3>Personal especializado:</h3>
        <p>
            Las empresas que se encuentran en esta fase tienen un buen dominio de sus capaciades actuales y sus carencias. Por lo general, la adquisición de conocimientos técnicos viene determinada por un plan estructurado de certificación y formación con objetivos definidossegún las diferentes unidades de negocio.
        </p>
        <h3>Recomendaciones</h3>
        <h5>Medición:</h5>
        <p>
            Debes intentar seguir mejorando con respecto a los sistemas de medición y establecer métricas e indicadores avanzados para medir los resultados de la nube híbrida, la satisfacción del cliente habitual y la eficacia de la prestación de servicios, incluidas la implementación y el soporte.
        </p>
        <h5>Optimization:</h5>
        <p>
            Es importante que incorpores métricas e indicadores para optimizar la prestación de servicios. Esto permite incentivar a los equipos de operaciones por su eficiencia en beneficio de la satisfacción del cliente. A medida que tu negocio en la nube empiece a ampliarse, la eficiencia marcará una auténtica diferencia en tus resultados finales.
        </p>
        <h5>Analítica:</h5>
        <p>
            Las funciones de análisis deben seguir evolucionando en las áreas clave del negocio: finanzas, ventas, prestación de servicios, marketing y CRM. A medida que avances hacia la siguiente fase de desarrollo, esto constituirá un factor diferenciador clave.
        </p>
        ',
        'stage5'=>'
        <p>
            Enhorabuena. Según la evaluaciónde IDC, tu organización se encuentra en la fase 5 en lo que respecta al desarrollo de tus procesos y conocimientos técnicos en la nube. Solo el 8 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría está en la fase 2. De forma general, en esta fase de su desarrollo, las organizaciones comparten las siguientes características:
        </p>
        <h3>Medición:</h3>
        <p>
            Las organizaciones disponen de métricas y KPIs bien definidos en todas las áreas clave del negocio. Los sistemas y los procesos están diseñados para optimizar la rentabilidad de la nube.
        </p>
        <h3>Propiedad intelectual interna:</h3>
        <p>
            En muchos casos, las organizaciones que se encuentran en esta fase venden y proporcionan su propiomodelo SaaS. De esta forma, logran márgenes elevados y garantizan que sus objetivos de ingresos recurrentes se alcancen y puedan ser medidos.
        </p>
        <h3>Automatización:</h3>
        <p>
            Las organizaciones que se encuentran en esta fase han automatizado totalmente muchos de los procesos en la nube, optimizando las soluciones en la nube y generando economías de escala en todos los aspectos del negocio. La automatización está estrechamente vinculada a funciones analíticas sólidas, incluidas el rendimiento de las aplicaciones y su porcentaje de uso por parte de los clientes.
        </p>
        <h3>Personal especializado:</h3>
        <p>
            En esta fase de desarrollo, las empresas han conseguido certificaciones en los niveles más altos de los programas de proveedores. Generalmente, estas disponen de las mejores estructuras y herramientas de prestación de servicios.
        </p>
        <h3>Recomendaciones</h3>
        <h5>Escalabilidad y alcance:</h5>
        <p>
            Incluso en este alto nivel de desarrollo de los procesos, debe seguir mejorando los sistemas y los servicios para que funcionen a escala, adquiriendo una mayor especialización con niveles de automatización de los procesos más altos. De este modo, reducirás los plazos para completar proyectos, lo que genera una mayor rentabilidad y más crecimiento. Debes hacer más hincapié en la escalabilidad, garantizando que la empresa pueda cumplir con sus compromisos y aprovechando al máximo sus amplias competencias.
        </p>
        <h5>Automatización:</h5>
        <p>
            Si garantizas que la automatización se integra en todas las áreas del negocio podrás evaluarel porcentaje de uso de las aplicaciones por parte del cliente y optimizar la prestación de servicios en la nube, así como agilizar y sacar el máximo partido de los procesos de venta y entrega.
        </p>
        <h5>Analítica:</h5>
        <p>
            Debes empezar a aprovechar algunas de las funciones de análisis para proporcionar valor a los clientes, como, por ejemplo, la de análisis predictivos de la utilización de recursos.
        </p>
        <h5>Transformación digital:</h5>
        <p>
            En esta fase, tus iniciativas yplanes de formación deben centrarse en la especialización para alcanzar objetivos de transformación digital más amplios, incluidos los procesos digitales, de integración y de negocio.
        </p>
        ',
    ],
    'marketing' => [
        'title'=>'Funciones de marketing',
        'pageimage' => '',
        'stage1'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 1 en lo que respecta al desarrollo de tus funciones de marketing en la nube. El 28 % de las organizaciones que han participado en nuestro estudiose encuentran en esta fase. La mayoría está en la fase 1. De forma general, en esta fase de su desarrollo, las organizaciones comparten las siguientes características:
        </p>
        <h3>Actividades de marketing:</h3>
        <p>
            Principalmente utilizanmétodos tradicionales (eventos, documentación y patrocinios). Por consiguiente, el éxito del marketing surge de la combinación de actividades del proveedor y algunas acciones localizadas con poco control sobre dónde y cómo se realizan.
        </p>
        <h3>Coordinación con ventas:</h3>
        <p>
            Normalmente, en esta fase, las organizaciones se basan en las comunicaciones externas (outbound) para captar nuevos clientes, con pocos indicadores para medir el éxito o cualificación de clientes potenciales (leads). También hay pocos indicadores de ajustes para mejorar.
        </p>
        <h3>Liderazgo de opinión:</h3>
        <p>
            En esta fase, las empresas se suelen centrar en el marketing más tradicional y no crean de forma proactiva contenido de liderazgo de opinión, como blogs, tutoriales en vídeo y artículos técnicos.
        </p>
        <img src="http://sage.idcready.net/images/tools/1/marketing.jpg" class="right">
        <h3>Recomendaciones</h3>
        <h5>Transformación de marketing:</h5>
        <p>
            Para impulsar el éxito futuro en la nube, tu organización debe formular un nuevo planteamiento de marketing y emplear métodos mucho más sofisticados. El marketing digital será esencial en este proceso, pero requiere un cambio de mentalidad de toda la organización. Crea un equipo de trabajo formado por personal directivo, de productos, de marketing y de ventas para guiar y gestionar tu transformación de marketing.
        </p>
        <h5>Segmentar y evaluar:</h5>
        <p>
            Necesitassegmentar y evaluar tu actualcartera de clientes, y centrarte en las áreas que proporcionan éxito en el marketing y en métodos que hayan proporcionado anteriormente el mejor feedback. En vez de un método aleatorio de las comunicaciones, es recomendable que actualicesel contenido y destines los mensajes a destinatarios específicos. Para lograrlo, debes empezar a seguir las estrategias de marketing y colaborar con los proveedores para comunicar mejor las propuestas de la nube.
        </p>
        <h5>Contenido basado en la experiencia de la nube:</h5>
        <p>
            Debes empezar a dar ejemplo de cara a los clientes, muchos de los cuales también se encontrarán en las primeras fases de adopción de la nube. Esto implicaasesoramiento, contenido formativo y la comunicación clara de las ventajas y desventajas de la nube con respecto a las instalaciones on-premise. Aprovecha el contenido que tus Partners proveedores ponen a tu disposición, pero adáptalo con tu propiamarca y personalízalo según tu mercado y destinatarios.
        </p>
        ',
        'stage2'=>'
        <p>
            Según la evaluaciónde IDC, tu organización se encuentra en la fase 2 en lo que respecta al desarrollo de las funciones marketing en la nube. El 26 % de las organizaciones que han participado en nuestro estudiose encuentran en esta fase. La mayoría de ellas están en la fase 1. Por lo general, en esta fase de su desarrollo, las organizaciones comparten las siguientes características:
        </p>
        <h3>Actividades de marketing:</h3>
        <p>
            Las organizaciones que se encuentran en esta fase se centran, sobre todo, en métodos tradicionales (eventos, documentación y patrocinios), pero empiezan a explorar los canales de marketing digital.
        </p>
        <h3>Coordinación con ventas:</h3>
        <p>
            Es muy probable que las organizaciones en esta fase realicen una combinación de sus propias actividades de generación de demanda y de actividades conjuntas con sus proveedores. De este modo, puedencualificar a sus clientes potenciales, así como evaluar y actualizar continuamente su planteamiento.
        </p>
        <h3>Liderazgo de opinión:</h3>
        <p>
            En esta fase, las empresas se suelen centrar en el marketing más tradicional y no crean de forma proactiva contenido de liderazgo de opinión, como blogs, tutoriales en vídeo y artículos técnicos.
        </p>
        <img src="http://sage.idcready.net/images/tools/1/marketing.jpg" class="right">
        <h3>Recomendaciones</h3>
        <h5>Definir unaexperiencia de la nube:</h5>
        <p>
            Sigue desarrollando tus funciones de marketing digital en toda la organización como parte de un proceso más amplio de transformación del marketing. Como parte de este proceso, es recomendable que adaptes el marketing a la experiencia de los clientes con la nube. De este modo se garantiza que la organización pueda definir su propuesta de valor y generar actividades de marketing en cada fase de esta experiencia del cliente.
        </p>
        <h5>Métricas de marketing:</h5>
        <p>
            Las métricas de marketing se deben crear en torno a la cualificación y generación de leads. También se deben adaptar a cada actividad y canal de marketing, como el marketing tradicional basado en eventos, el outboundmarketing y el marketing digital. Por lo demás, dichas métricas se deben definir con respecto a objetivos de marketing más amplios, como el reconocimiento, la imagen de marca y la generación de demanda.
        </p>
        <h5>Liderazgo de opinión:</h5>
        <p>
            En esta fase conviene que personalices el contenidoque tus Partners proveedores han puesto a tu disposición. También debes empezar a crear un plan para tus propias actividades de liderazgo de opiniónen el que interactúes directamente con tu mercado y destinatarios. Esto incluye blogs, foros, documentos y referencias.
        </p>
        ',
        'stage3'=>'
        <p>
            Según la evaluaciónde IDC, tu organización se encuentra en la fase 3 en lo que respecta al desarrollo de tus funciones de marketing en la nube. El 26 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría de ellas están en la fase 1. Por lo general, en esta fase de su desarrollo, las organizaciones comparten las siguientes características:
        </p>
        <h3>Actividades de marketing:</h3>
        <p>
            Las organizaciones que se encuentran en esta fase conocen bien lasnecesidades del cliente y se comunican a través de los canales adecuados. También utilizan métricas y campañas de seguimiento según objetivos predeterminados.
        </p>
        <h3>Coordinación con ventas:</h3>
        <p>
            En esta fase, las empresas han comenzado a desarrollar mensajes que tienen repercusión en los clientes de la nube y les guían a través del ciclo de ventas. Eso implica una mayor coordinación con el departamento de ventas, haciendo que muchas organizaciones ya no se basen en las campañas de proveedores para impulsar la generación de demanda.
        </p>
        <h3>Liderazgo de opinión:</h3>
        <p>
            En esta fase, aunque de un modo limitado, las empresas han empezado a personalizar el contenido que sus Partners proveedores han puesto a su disposición, adaptándolo a sus destinatarios. Algunas han empezado a crear blogs y a usar otros foros sociales, pero las actividades suelen ser ad hoc e impulsadas por iniciativas individuales en vez de hacerlo la empresa en conjunto.
        </p>
        <img src="http://sage.idcready.net/images/tools/1/marketing.jpg" class="right">
        <h3>Recomendaciones</h3>
        <h5>Crear funciones de marketing digital:</h5>
        <p>
            Llegados a este punto, resulta imperativo aumentar la presencia on-line. Comienza creando un plan de marketing digital que tenga en cuenta la incorporación (o la colaboración) de recursos digitales y de una serie de actividades digitales. Entre ellas se encuentra la búsqueda de notoriedad, la captación de clientes online y el establecimiento de objetivos de liderazgo de opinión. También es recomendable que actualicestu página web y analices e idees formas para que los clientes la visiten mediante contenido relevante y útil que ofrezca asesoramiento.
        </p>
        <h5>Impulsar el ciclo de ventas:</h5>
        <p>
            Debes considerar el marketing más que como una forma de captar el interés de clientes potenciales, como un modo de ofrecer a los equipos comerciales clientes potenciales cualificados. Esto significa que debes poder ofrecer la información, la formación y el contenido adecuados a una serie de clientes potenciales de la nube a través de un conjunto de herramientas avanzadas de contenido digital. También se requiere un departamento estructurado para crearlas y mantenerlas.
        </p>
        <h5>Métricas y KPIs:</h5>
        <p>
            La segmentación permitirá que tu organización disponga de uncoste por cierre para tu negocio en la nube, y que se beneficie de una combinación de planteamientos digitales, de redes sociales y basados en contenido. Es recomendable que empieces creando métricas y KPIs muy claros para todas las actividades de marketing (tanto tradicionales como digitales), que no se limiten exclusivamente a los clientes captados.
        </p>
        ',
        'stage4'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 4 en relación con el desarrollo de las funciones de marketing en la nube. El 12 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría de ellas están en la fase 1. Por lo general, en esta fase de su desarrollo, las organizaciones comparten las siguientes características:
        </p>
        <h3>Actividades de marketing:</h3>
        <p>
            En esta fase, las organizaciones han desarrollado una segmentación avanzada de su público objetivo, utilizando software y herramientas de automatización de marketing. De este modo, pueden enviar el contenido adecuado a cada destinatario.
        </p>
        <h3>Coordinación con ventas:</h3>
        <p>
            El marketing digital funciona esencialmente como una extensión del proceso de ventas. De este modo, el área de marketing puede proporcionar clientes potenciales a los que se ha guiadopor gran parte del ciclo de ventas con contenido digital que han encontrado por sí mismos.
        </p>
        <h3>Liderazgo de opinión:</h3>
        <p>
            En esta fase, las organizaciones son artífices del contenido y de las campañas de liderazgo de opinión, a la vez que proporcionan materiales que sirven de formación al mercado objetivo y posicionan a la empresa como líder en ese campo.
        </p>
        <img src="http://sage.idcready.net/images/tools/1/marketing.jpg" class="right">
        <h3>Recomendaciones</h3>
        <h5>Continuar desarrollando medición:</h5>
        <p>
            Gracias a unamedición activa de todos los aspectos y elementos del marketing, podrás conocer mejor lo que funciona bien y lo que no. En concreto, qué se puede mejorar y qué se debe desarrollar a medida que cambie el negocio. La medición exhaustiva también te permitirá conocer los resultados de las campañas de marketing para ganar en eficiencia.
        </p>
        <h5>Coordinar ventas y marketing:</h5>
        <p>
            El equipo de marketing debería actuar como una extensión del de ventas, compartiendo información y garantizando una colaboración estrecha entre ambos departamentos. Las necesidades de madurez de ventas debenapoyarse en una combinación de planteamientos digitales, de redes sociales y basados en contenido. Esto incluye el análisis de los clientes y la identificación de personal específico dentro de las organizaciones.
        </p>
        <h5>Incentivar a los líderes de opinión de la empresa:</h5>
        <p>
            Debes tratar de identificar los posibles líderes de opinión y asesores de confianza en las diferentes unidades de negocio (ventas, marketing, servicios, soluciones o equipo directivo) e incentivarlos para que participen en las actividades de liderazgo de opinión: dar ponencias en eventos, escribir blogs, crear vídeos y generar contenido para redes sociales, entre otras.
        </p>
        ',
        'stage5'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 5 en lo que respecta al desarrollo de las funciones de marketing en la nube. Solo el 8 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría de ellas están en la fase 1. Por lo general, en esta fase de su desarrollo, las organizaciones comparten las siguientes características:
        </p>
        <h3>Actividades de marketing:</h3>
        <p>
            Las organizaciones que se encuentran en esta fase deben recurrir a una combinación de planteamientos digitales, de redes sociales y basados en contenido. Esto incluye el análisis de los clientes y la identificación de público objetivo. Muchas organizaciones pueden conseguirun objetivo de coste por cierre mucho más bajo, utilizando un análisis exhaustivo de los resultados de la campaña para ganar en eficiencia.
        </p>
        <h3>Coordinación con ventas:</h3>
        <p>
            En esta fase, hay una colaboración muy estrecha entre los equipos comerciales y de marketing, y este último se considera una extensión deldepartamento comercial y se trata (e incentiva) en consecuencia.
        </p>
        <h3>Liderazgo de opinión:</h3>
        <p>
            A las organizaciones que se encuentran en esta fase se les considera líderes de opinión de confianza y asesores estratégicos. Las actividades de liderazgo de opinión son continuas y el contenido formativo se proporciona de un modo frecuente y a través de varios canales.
        </p>
        <img src="http://sage.idcready.net/images/tools/1/marketing.jpg" class="right">
        <h3>Recomendaciones</h3>
        <h5>Evolución y desarrollo continuos:</h5>
        <p>
            Los procesos de segmentación, seguimiento y métricas avanzadas requieren una evolución continua. Con la incorporación al mercado de nuevas herramientas, es recomendable que las utilices continuamente para estar a la última.
        </p>
        <h5>Eficacia del marketing:</h5>
        <p>
            Continúa reduciendo el coste por cierre, e interactúa de forma activa con los clientes para generar contenido de referencia y ampliar las actividades de marketing.
        </p>
        <h5>Convertirse en evangelistas de la nube:</h5>
        <p>
            Conduce el liderazgo de opinión y el asesoramiento estratégico hacia la «evangelización». Empieza a contratar, identificar y formar a «evangelizadores» internos (dentro de tu propia organización) y externos, en los medios de comunicación, la comunidad de analistas, los Partners y, de preferencia, en tu cartera de clientes.
        </p>
        ',
    ],
    'sales' => [
        'title'=>'Sales',
        'pageimage'=>'sales.jpg',
        'stage1'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 1 por lo que respecta alas ventas en la nube. El 21 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría de ellas están en la fase 2. De forma general, en esta fase de desarrollo, las organizaciones comparten las siguientes características:
        </p>
        <h3>Personal:</h3>
        <p>
            Es probable que tengan un número reducido de comerciales con certificación y orientados a la venta de servicios en modo SaaS, pues la mayor parte de la actividad comercial se centra en soluciones on-premise. Los equipos comerciales carecen de objetivos específicos relativos a la nube y, con frecuencia, las estructuras de incentivos de las ventas de SaaS por suscripción no están definidas de forma adecuada.
        </p>
        <h3>Capacitación:</h3>
        <p>
            Mientras que es probable que las medidas para incrementar la venta de SaaS sean ad hoc y estén centradas en oportunidades específicas, es poco probable que los comerciales en esta fase de desarrollo estén aprovechando al máximo las herramientas, la documentación y otras fuentes de información que proporcionan los Partners proveedores.
        </p>
        <h3>Estrategia comercial:</h3>
        <p>
            Todavía es pronto para que la venta de SaaS constituya una parte esencial de la estrategia comercial y, por lo general, su venta responde a una necesidad urgente para cubrir la demanda del cliente.
        </p>
        <h3>Recomendaciones</h3>
        <h5>Especialistas en ventas:</h5>
        <p>
            Empieza creando un equipo de especialistas en preventas orientados a la nube, con los conocimientos técnicos necesarios para cerrar negocios específicos junto con los Account Managers.
        </p>
        <h5>Incentivos y comisiones:</h5>
        <p>
            A medida que tu organización adopte la nube, necesitarás migrar las ventas de las soluciones on-premise a modelos de negocio basados en ingresos recurrentes. Las estructuras de incentivos y comisiones se deberán ajustar a esta nueva situación. Para lograrlo, es fundamental que el equipo comercial se oriente a la renovación y a la consolidación de relaciones a más largo plazo. Te recomendados que empieces a implementar los cambios en las estructuras de comisiones al principio, con la compensación anticipada (un porcentaje de los ingresos del primer año) como el modelo de remuneración más habitual.
        </p>
        <h5>Documentación y formación:</h5>
        <p>
            La formación debe centrarse en consolidar los conocimientos técnicos necesarios para vender SaaS de un modo efectivo. Es esencial conocer las ventajas para los clientes y tener los argumentos suficientes para superar cualquier objeción. Facilita a los comerciales la identificación de las herramientas de formación que se necesitan en esta fase y haz que la asistencia a cursos de formación sea un KPI clave.
        </p>
        ',
        'stage2'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 2 por lo que respecta a las ventas en la nube. El 28 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría de ellas también están en la fase 2. De forma general, las organizaciones que se encuentran en esta fase de desarrollo comparten las siguientes características:
        </p>
        <h3>Personal:</h3>
        <p>
            Las organizaciones comienzan a crear un equipo de especialistas con los conocimientos técnicos necesarios para cerrar ventas en la nube. Las estructuras de comisiones han evolucionado de un porcentaje de los ingresos del primer año o una comisión directa por ingresos recurrentes. Algunas organizaciones combinan ambos modelos, pero muy pocas han aplicado modelos de incentivos adicionales por las renovaciones o por el porcentaje de uso por parte de los clientes, si bien es una prioridad.
        </p>
        <h3>Capacitación:</h3>
        <p>
            Los comerciales empiezan a usar de forma activa los recursos para incrementar la productividad de las ventas que tienen a su disposición (normalmente a través de portales de los Partners proveedores). También obtienen formación y certificación a través de sus principales Partners proveedores.
        </p>
        <h3>Estrategia comercial:</h3>
        <p>
            La nube se está convirtiendo en algo fundamental para la estrategia comercial y suele dar paso a otras oportunidades de venta (upsell). Se recomienda facilitar a los comerciales las técnicas de upsell, añadiendo la prestación de servicios adicionales a los contratos de SaaS. En adelante, las organizaciones deben tratar de incorporar las oportunidades de upsell como un componente básico de su estrategia comercial. 
        </p>
        <h3>Recomendaciones</h3>
        <h5>Comisiones:</h5>
        <p>
            Mejora las estructuras de comisiones para incentivar a los comerciales en el ciclo completo del cliente y céntrate en la satisfacción del cliente para que renueve los contratos.
        </p>
        <h5>Orientación a las ventas en la nube:</h5>
        <p>
            Debes integrar las ofertas en la nube en tus estrategias y competencias comerciales, convirtiéndolo en el enfoque básico de la empresa y un aspecto fundamental en tus conversaciones con clientes. Aprovecha los kits de ventas de los proveedores, crea tus propias guías y realiza una formación continua.
        </p>
        <h5>Upselling de servicios y soluciones:</h5>
        <p>
            A medida que el negocio en la nube empiece a ganar impulso, facilita a los comerciales la venta adicional de servicios y soluciones relacionadas. Asocia las ofertas de servicios estándar, como la formación y el soporte, a tu negocio de SaaS. Empaquétalas y factúralas por suscripción, para que se puedan vender junto con la suscripción a SaaS.
        </p>
        ',
        'stage3'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 3 respecto a las ventas en la nube. El 27% de las organizaciones que han participado en este estudiose encuentran en esta fase. La mayoría de ellas están en la fase 2. Por lo general, las organizaciones que se encuentran en esta fase de desarrollo comparten las siguientes características:
        </p>
        <h3>Personal:</h3>
        <p>
            Las ventas en la nube están respaldadas por personal de preventa y de venta cualificado, capaz de ofrecer soluciones en la nube para satisfacer las necesidades de los clientes. En esta fase de desarrollo, las estructuras de incentivos para la nube están bien definidas, pero no se extienden al ciclo completo de un cliente.
        </p>
        <h3>Capacitación:</h3>
        <p>
            Los equipos comercialestienen cada vez más los conocimientos técnicos y las certificaciones que requiere la nube. Además, se ha ampliado la formación y la participación activa en las actividades de generación de demanda por parte de los departamentos de marketing internos y las iniciativas de los Partners proveedores. 
        </p>
        <h3>Estrategia comercial:</h3>
        <p>
            La nube se ha integrado en todas las propuestas del equipo comercial. La mayor parte de los comerciales puede asesorar a los clientes sobre la nube de un modo competente y presentar un presupuesto con opciones on-premise y en la nube.
        </p>
        <h3>Recomendaciones</h3>
        <h5>Perfeccionamiento comercial:</h5>
        <p>
            Mejora las estructuras de comisiones para incluir KPIs específicos, como los resultados de satisfacción de clientes e incentivos adicionales por captación de nuevos clientes y porcentaje de renovaciones. A medida que evolucione la estructura, deberás consolidar los conocimientos técnicos del equipo comercial en las líneas de negocio, lo que te permitirá estrechar aún más las relaciones con los clientes principales, así como impulsar tu especialización en los mercados clave.
        </p>
        <h5>Escalabilidad:</h5>
        <p>
            Si tu empresa ofrece SaaS tanto de volumen como de valor, te recomendamos que empieces a diferenciar las estrategias de SaaS de volumen frente a las de valor. En el caso de SaaS de volumen, la capacidad de escalar empieza a ser fundamental para el crecimiento. Tu estrategia comercial basada en el volumen debe incluir un plan de inversión para la automatización de ventas, así como la revisión del equipo de telemarketing. Invierte en herramientas que generen eficacia en el proceso de ventas (herramientas de automatización). Tu estrategia comercial basada en el valor debe abarcar diferentes líneas de negocio, e incluir nuevas incorporaciones, así como la adquisición de conocimientos técnicos a través de la formación.
        </p>
        <h5>Desarrollo de pruebas de concepto:</h5>
        <p>
            Llegados a este punto, te recomendamos crear una estrategia de viabilidad o pruebas de concepto más formal y, si es posible, invertir en tus propias herramientas de demostración y prueba. De este modo, los nuevos clientes podrán probar SaaS en un plan de 30 días. Esto resulta una herramienta de ventas potente, sobre todo si puede incorporar datos del cliente y reforzar las funciones de preventa para apoyar el proceso de ventas.
        </p>
        ',
        'stage4'=>'
        <p>
            Según la evaluaciónde IDC, tu organización se encuentra en la fase 4 por lo que respecta a las ventas en la nube. El 16 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría de ellas están en la fase 2. Por lo general, las organizaciones que se encuentran en esta fase de desarrollo comparten las siguientes características:
        </p>
        <h3>Personal:</h3>
        <p>
            Las organizaciones han integrado la nube en el catálogo de soluciones que ofrece su equipo comercial, mediante actividades avanzadas de soporte de preventa y herramientas de gestión dela pipeline y de las oportunidades. Las empresas con un modelo negocio de SaaS basado en el volumen están intentando aumentar su negocio mediante el telemarketing y la venta online, mientras que las que tienen una estrategia de SaaS de valor orientan su actividad a adquirir conocimientos técnicos para la venta según la línea de negocio.
        </p>
        <h3>Incentivos:</h3>
        <p>
            Las estructuras de incentivos para la nube están bien definidas y los comerciales también tienen más probabilidades de recibir incentivos durante el ciclo completo del cliente. Es probable que las estructuras de comisiones sigan evolucionando, y sean una combinación de incentivos anticipados y continuos.
        </p>
        <h3>Capacitación:</h3>
        <p>
            Normalmente, las herramientas de demostración y de viabilidad o prueba de concepto están a disposición de los clientes y se usan de forma activa como parte del proceso de preventa. Las empresas podrían invertir en ellas, aprovechar las de sus proveedores o ambas opciones. Las herramientas comerciales y la documentación se actualizan y renuevan constantemente.
        </p>
        <h3>Estrategia comercial:</h3>
        <p>
            La práctica recomendada es vender a los clientes utilizando varias opciones de entrega, con una combinación de estructuras de comisiones anticipadas y continuas. Los equipos comerciales tienen más probabilidades de vender en función de los resultados comerciales de la solución que por sus características. Asimismo, hay una creciente especialización en el modo en que los equipos comerciales se adaptan a los mercados y procesos de negocio. Muchas empresas están empezando a crear soluciones y departamentos comerciales con estructura vertical.   
        </p>
        <h3>Recomendaciones</h3>
        <h5>Experiencia en el sector:</h5>
        <p>
            A medida que evoluciones, intenta contratar a comerciales que ya no estén en el sector tecnológico y que estén centrados en el proceso de negocio, lo que te permitirá estrechar aún más las relaciones con tus clientes principales, así como impulsar tu especialización en los mercados clave.   
        </p>
        <h5>Evolución de los incentivos:</h5>
        <p>
            A medida que sigas evolucionando, los incentivos por ventas deben ser coherentes y centrarse cada vez más en proporcionar valor al cliente, independientemente de los modelos de entrega en la nube.
        </p>
        <h5>Integración de ventas y puesta en marcha:</h5>
        <p>
            Céntrate en una colaboración más estrecha y fluida entre los equipos comerciales y de puesta en marcha de servicios en la nube, lo que, en última instancia, genera crecimiento, escalabilidad y rentabilidad en todas las áreas del negocio. La automatización y la digitalización del proceso de ventas serán esenciales para conseguirlo. Asimismo, los incentivos por ventas deben orientarse, de forma coherente, a proporcionar valor al cliente, independientemente de los modelos de prestación de servicios en la nube.
        </p>
        <h5>Innovación junto a los proveedores:</h5>
        <p>
           Los equipos de ventas y preventas deben poder trabajar con proveedores ofrecer soluciones innovadoras de forma conjunta. El crecimiento, la escalabilidad y la rentabilidad en todas las áreas del negocio, así como la automatización y la digitalización del proceso de ventas, serán esenciales para crecer más.
        </p>
        <h5>Transformación digital:</h5>
        <p>
            También en esta fase es esencial que incluyas tus competencias relativas a la nube en una propuesta de transformación digital más amplia. En dicha propuesta se incluye un gran número de tecnologías digitales, como la movilidad, las redes sociales, los análisis de Big Data, la seguridad de nueva generación y el Internet de las Cosas (IoT, por sus siglas en inglés).
        </p>
        ',
        'stage5'=>'
        <p>
            Según la evaluaciónde IDC, tu organización se encuentra en la fase 5 respecto a las ventas en la nube. Solo un 8 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría de ellas están en la fase 2. Por lo general, las organizaciones que se encuentran en esta fase de desarrollo comparten las siguientes características:
        </p>
        <h3>Personal:</h3>
        <p>
            El departamento comercial puede vender soluciones, servicios y consultoría basándose en resultados empresariales. Normalmente, las organizaciones han contratado personal especializado en la venta de soluciones verticales para consolidar su posición en determinados mercados, lo cual les ha permitido retener a los principales clientes. En esta fase de desarrollo, es probable que los modelos de negocio en la nube evolucionen y lleguen a ser uno de los pilares clave de la transformación digital.
        </p>
        <h3>Incentivos:</h3>
        <p>
            Las organizaciones disponen de estructuras de comisiones por ventas avanzadas, orientadas a proporcionar valor a los clientes, independientemente de los modelos de entrega en la nube. En ellas se incluyen la satisfacción del cliente, la captación de nuevos clientes y los porcentajes de renovación. Con todo lo conseguido, es esencial que ahora las organizaciones aprovechen al máximo su posición en el mercado.
        </p>
        <h3>Capacitación:</h3>
        <p>
            Las actividades de formación están centradas principalmente en la transformación digital. Deben abarcar la venta de soluciones omnicanal, adquirir un conocimiento más exhaustivo del proceso de negocio, poder comunicar y proporcionar valor empresarial, y vender a clientes de línea de negocio.
        </p>
        <h3>Estrategia comercial:</h3>
        <p>
            Es probable que las organizaciones ya estén trabajando con proveedores para ofrecer soluciones innovadoras y que hayan establecido colaboraciones estratégicas con proveedores y clientes. En última instancia, el área comercial de las organizaciones empezará a proporcionar la economía de escala que han estado buscando, lo que les permitirá impulsar la rentabilidad en todas las áreas del negocio y dirigir a los comerciales hacia las nuevas oportunidades de crecimiento.
        </p>
        <h3>Recomendaciones</h3>
        <h5>Transformación digital:</h5>
        <p>
            La interacción con el cliente debe centrarse en adquirir los conocimientos técnicos para la venta y transformación digital. Debe haber una estrecha colaboración entre los departamentos de marketing y comercial, en la que el departamento de marketing funcione como una extensión del comercial, en concreto, para la captación de nuevos clientes.
        </p>
        <h5>Contratación de personal especializado: </h5>
        <p>
           La contratación de comerciales especializados en ventas de soluciones verticales te ayudará a consolidar tu posición en determinados mercados, lo que te permitirá retener a tus principales clientes durante varios años y hacer crecer más el negocio. Considera también la posibilidad de contratar personal con competencias no tradicionales como conocimientos especializados en el campo digital, en bases de datos y en análisis avanzados.
        </p>
        <h5>Colaboraciones más estrechas:</h5>
        <p>
            Establece colaboraciones con proveedores estratégicos, así como con clientes. Esto te ayudará a garantizar una evolución continua del proceso de ventas en la empresa.
        </p>
        <!--h5>Co-innovation with vendors:</h5>
        <p>
            Sales teams and presales must be able to work with vendors to co-innovate solutions. Ultimately, you will be able to drive growth, scalability, and profitability into all areas of the business, and automation and digitization of the sales process will be central to achieving this.
        </p>
        <h5>Transformación digital:</h5>
        <p>
            Also critical at this stage is embedding your cloud capabilities into a broader digital transformation proposition. This incorporates a breadth of digital technologies, such as mobility, social, Big Data analytics, next-gen security, and the Internet of Things (IoT).
        </p-->
        ',
    ],
    'business-mix' => [
        'title'=>'Mix de negocio',
        'pageimage' => '',
        'stage1'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 1 respecto al mix de negocio en la nube. El 4% de las organizaciones que han participado en nuestro estudio se encuentra en esta fase. La mayoría de ellas están en la fase 3. Por lo general, las organizaciones que se encuentran en esta fase de desarrollo comparten las siguientes características:
        </p>
        <h3>Contribución a los ingresos: </h3>
        <p>
            La mayor parte de los ingresos de software proceden de la venta de licencias on-premise. En esta fase, muy pocas empresas tienen objetivos firmes para fomentar cualquier tipo de ingresos recurrentes.
        </p>
        <h3>Programas para Partners:</h3>
        <p>
            De forma general, las organizaciones están en el nivel más bajo de los programas de proveedores específicos para la nube (aunque puedan estar en niveles más altos en los productos on-premise). Proveedores como Sage están aumentando sus inversiones en la captación de nuevos Partners, incentivos financieros y planes de formación específica para los modelos SaaS y otros servicios en la nube.
        </p>
        <h3>Servicios:</h3>
        <p>
            Aunque en esta fase las organizaciones puedan ofrecer servicios relacionados con el software, como consultoría, personalización, implementación, soporte y formación, en muy pocas ocasiones están adaptados o diseñados específicamente para mejorar sus modelos de negocio SaaS u otros servicios en la nube.
        </p>
        <h3>Infraestructura en la nube:</h3>
        <p>
            En esta fase, las organizaciones aún tienen que realizar inversiones importantes en sus propias infraestructuras, herramientas y tecnologías de gestión de la nube. Por consiguiente, es muy poco probable que vayan más allá de la distribucióny prescripción básica. Esto se debe principalmente a que no disponen de la capacidad para gestionar entornos híbridos complejos o complementos de valor adicional, como la facturación unificada, la gestión y la seguridad.
        </p>
        <h3>Soluciones: </h3>
        <p>
            "En esta fase, muy pocas empresas han desarrollado soluciones específicas para la nube (horizontales o verticales) en colaboración con los Partners proveedores, o creado su propia propiedad intelectual en la nube. 
Recomendaciones"
        </p>
        <!--<img src="http://sage.idcready.net/images/tools/1/business-mix.jpg" class="right">-->
        <h3>Recomendaciones</h3>
        <h5>Desarrollar un plan financiero:</h5>
        <p>
            Incorpora los ingresos recurrentes en tu negocio. Si no lo haces, puede que repercuta en la tesorería de la empresa al cambiar del modelo basado en proyectos al de ingresos recurrentes. Genera ingresos recurrentes a lo largo del tiempo, creando un plan estructurado y adaptando tu gama de productos y servicios, así como tus inversiones en infraestructura en cada fase. Trata de forma activa de diversificar los ingresos y de reducir la dependencia en los ingresos de la distribución. Para ello, deberás efectuar una evaluación y una planificación cuidadosas de tu abanico de ofertas de servicios, infraestructura y soluciones en la nube.
        </p>
        <h5>Selección de proveedores:</h5>
        <p>
            Evalúa a tus Partners proveedores de la nube y da prioridad a aquellos que sean más estratégicos para tu negocio futuro, de acuerdo a tus objetivos de mercado y mix de negocio. Elabora un plan de desarrollo con esos proveedores estratégicos para subir al siguiente nivel en sus Programas para Partners de aplicaciones y servicios en la nube. 
        <h5>Adaptar la cartera de servicios:</h5>
        <p>
            A medida que avances hacia la siguiente fase, te recomendamos que adaptes tu cartera de servicios a tu negocio en la nube y al modelo SaaS, y crea oportunidades de upselling. El margen por los servicios es normalmente superior al de la distribución y la referencia de clientes. La creación de modelos de negocio basados en servicios rentables relacionados con su negocio de venta en la nube te ayudará a consolidar resultados netos. En tu progreso hacia las siguientes fases, cuanto más automatizados y repetibles sean los servicios, más rentables te resultarán.
        </p>
        ',
        'stage2'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 2 respecto a tu mix de negocio en la nube. El 16 % de las organizaciones que han participado en nuestro estudio se encuentra en esta fase. La mayoría está en la fase 3. Por lo general, las organizaciones que se encuentran en esta fase de desarrollo comparten las siguientes características:
        </p>
        <h3>Contribución a los ingresos: </h3>
        <p>
            Aunque la mayor parte de los ingresos por software proceden de la venta de licencias on-premise, el aumento de los ingresos recurrentes supera a los del modelo tradicional. En esta fase, las organizaciones acaban de empezar a evaluar su contribución a los ingresos recurrentes en relación con el modelo de negocio basado en proyectos. La mayoría se encuentra en las primeras fases de elaboración de un plan financiero.
        </p>
        <h3>Programas para Partners:</h3>
        <p>
            En esta fase de desarrollo, las organizaciones apuestan por Partners proveedores estratégicos para su negocio en la nube, y a menudo buscan proveedores de un nivel superior con programas específicos en la nube (aunque puedan estar en niveles más altos en los productos on-premise).
        </p>
        <h3>Servicios:</h3>
        <p>
            Es probable que las organizaciones estén desarrollando una cartera de servicios diseñada específicamente para crear oportunidades de upselling con el modelo SaaS. Normalmente, las organizaciones se centran en la formación, la implementación y el soporte. A medida que el negocio en la nube gana impulso, empiezan a consolidar su experiencia y la propiedad intelectual.
        </p>
        <h3>Infraestructura en la nube:</h3>
        <p>
            En esta fase, es poco probable que las organizaciones hayan efectuado inversiones importantes en herramientas e infraestructura en la nube y tecnología de gestión propias. Sin embargo, muchas empiezan a crear un plan tecnológico bien estructurado para conseguir tanto la tecnología como las competencias necesarias.
        </p>
        <!--<img src="http://sage.idcready.net/images/tools/1/business-mix.jpg" class="right">-->
        <h3>Recomendaciones</h3>
        <h5>Plan financiero:</h5>
        <p>
            En esta fase, es recomendable que establezcas un proceso de revisión periódica en el que se supervise la contribución a los ingresos recurrentes, los ingresos por servicios y los ingresos por venta de productos a lo largo del tiempo. Elabora un plan de inversión para la escalabilidad de la infraestructura y las herramientas de la nube que aporte valor añadido. Con ello se facilitará la puesta en marcha eficiente de servicios, como los servicios gestionados proactivos y la gestión híbrida.
        </p>
        <h5>Desarrollo de la propiedad intelectual:</h5>
        <p>
            Analiza los proyectos implementados e identifica las soluciones y los servicios tecnológicos que pueden convertirse en productos con su propia propiedad intelectual. Consulta quiénes son tus clientes actuales para determinar los sectores con oportunidad en los que tu empresa se puede especializar.
        </p>
        <h5>Colaboración con proveedores:</h5>
        <p>
            Analiza con tus Partners proveedores la oportunidad de desarrollar soluciones conjuntas (verticales u horizontales). También debes tratar de familiarizarte con los requisitos de nivel superior del programa para Partners y crear tu propio plan de negocio de acuerdo a dichos requisitos. La creación de un plan de negocio conjunto entre Partners y proveedores Cloud no solo te permitirá obtener más oportunidades, sino que mostrará tu compromiso.
        </p>
        <h5>Automatización y oportunidades de upselling:</h5>
        <p>
            Las empresas de éxito usan la nube como medio para desarrollar técnicas de upselling. Cuanto más automatizados y repetibles sean los servicios, más rentables serán. Incluso en esta fase inicial de tu experiencia con la nube, te aconsejamos que diseñes los futuros servicios pensando en la automatización.
        </p>
        ',
        'stage3'=>'
        <p>
            Según la evaluaciónde IDC, tu organización se encuentra en la fase 3 respecto a tu mix de negocio en la nube. El 53 % de las organizaciones que han participado en nuestro estudio se encuentra en esta fase. La mayoría de ellas también están en la fase 3. Por lo general, las organizaciones que se encuentran en esta fase comparten las siguientes características:
        </p>
        <h3>Contribución a los ingresos: </h3>
        <p>
            Los ingresos generados solo a partir de la nube se sitúan entre el 10 % y el 20 % de los ingresos totales. Los ingresos recurrentes están empezando a incrementarse y su impacto en la tesorería de la empresa se está empezando a notar.
        </p>
        <h3>Programas para Partners:</h3>
        <p>
            En esta fase, las organizaciones normalmente han identificado qué Partners proveedores son estratégicos para su negocio en la nubey están en el proceso de evolucionar a un nivel más alto de compromiso formando parte de programas específicos.
        </p>
        <h3>Servicios:</h3>
        <p>
            SaaS ofrece una opción completa que incorpora una serie de servicios basados en proyectos específicos (formación, consultoría, implementación y, tal vez, integración). En esta fase, muchas empresas han empezado a elaborar un catalogo de servicios gestionados unidos al negocio de la nube.
        </p>
        <h3>Infraestructura en la nube:</h3>
        <p>
            En esta fase, es probable que las organizaciones hayan establecido un plan tecnológico con una estructura adecuada. Han empezado a invertir en infraestructura, herramientas y tecnología de gestión de la nube para pasar de la distribución y prescripción básicas a un negocio en la nube más rentable.
        </p>
        <h3>Soluciones: </h3>
        <p>
            En esta fase, es muy poco probable que hayan desarrollado soluciones específicas (horizontales o verticales) o que hayan creado su propia propiedad intelectual de soluciones en la nube.
        </p>
        <!--<img src="http://sage.idcready.net/images/tools/1/business-mix.jpg" class="right">-->
        <h3>Recomendaciones</h3>
        <h5>Automatización:</h5>
        <p>
            Tienes que hacer hincapié en la creación y la ampliación de las funciones de automatización para los servicios gestionados referentes al modelo SaaS. El objetivo es desarrollar ofertas de servicios proactivas/preventivas. A medida que avances hacia las siguientes fases, cuanto más automatizados y repetibles sean los servicios, más rentable será tu negocio en la nube.
        </p>
        <h5>Aumentar las soluciones verticales y horizontales:</h5>
        <p>
            De este modo, tu negocio podrá lograr un mayor tamaño y alcance al estar dispuesto a invertir en especialización.
        </p>
        <h5>Soluciones junto con los Partners:</h5>
        <p>
            El modelo de negocio habitual y la planificación de cuentas con los Partners estratégicos garantizarán una estrecha colaboración y visibilidad. El plan tecnológico debería centrarse en adquirir los conocimientos técnicos para gestionar totalmente los entornos híbridos, a la vez que se da soporte a complementos de valor más complejos, como la facturación unificada, la optimización de redes, la gestión de la nube y la seguridad. Las soluciones futuras deben continuar siendo un tema de debate clave y se deben incorporar en los planes de negocio y tecnológicos conjuntos.
        </p>
        ',
        'stage4'=>'
        <p>
            Según la evaluación de IDC, tu organización se encuentra en la fase 4 en lo que respecta a tu mix de negocio en la nube. El 2 % de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría de ellas están en la fase 3. Por lo general, en esta fase de su desarrollo, las organizaciones comparten las siguientes características:
        </p>
        <h3>Contribución a los ingresos: </h3>
        <p>
            En esta fase, las empresas suelen generar más ingresos por la puesta en marcha de servicios que por la venta de productos o licencias. La nube tiende a contribuir, como mínimo, a un tercio de la facturación total, y la mayoría de las organizaciones disponen de sistemas de análisis e informes bastante detallados relativos a la nube. También conocen bien la contribución de los ingresos recurrentes a los ingresos totales.
        </p>
        <h3>Programas para Partners:</h3>
        <p>
            Las empresas que se encuentran en esta fase son capaces de ofrecer varios entornos y productos de proveedores, pero han establecido una relación estratégica con uno o varios proveedores de la nube. También están evolucionando, desarrollando una experiencia más avanzada con esos Partners proveedores estratégicos.
        </p>
        <h3>Servicios:</h3>
        <p>
            En esta fase, las empresas tienen una cartera de servicios para aprovechar las oportunidades de upselling que ofrece el modelo SaaS; entre las que se encuentran la consultoría, la formación, la implementación, el soporte y los servicios gestionados. A medida que avancen hacia las siguientes fases, cuanto más automatizados y repetibles sean los servicios, más rentables serán.
        </p>
        <h3>Infraestructura en la nube:</h3>
        <p>
            En esta fase, las empresas han invertido, y siguen invirtiendo, en infraestructura, herramientas y tecnología de gestión en la nube. La capacidad para gestionar e integrar varios entornos es un factor fundamental, y se da prioridad a las herramientas y a la arquitectura en la nube híbrida.
        </p>
        <!--<img src="http://sage.idcready.net/images/tools/1/business-mix.jpg" class="right">-->
        <h3>Recomendaciones</h3>
        <h5>Automatización:</h5>
        <p>
            Amplía las funciones de automatización para los servicios gestionados referentes al modelo SaaS. Dichos servicios se pueden automatizar y, por lo tanto, son repetibles y se pueden empaquetar y adjuntar a carteras del modelo SaaS para aprovechar las oportunidades de upselling. Generalmente, los servicios gestionados permiten la entrega remota.
        </p>
        <h5>Transformación digital:</h5>
        <p>
            Además de ofrecer soluciones, tienes que tener presente las transformaciones digitales que se producen en los diferentes modelos de negocios y sectores. En esta fase, debes estar al tanto del uso que hacen los clientes de la nube, con datos inmediatos de renovaciones preferentes y de uso real. Con el paso del tiempo, esto será cada vez más importante.
        </p>
        <h5>Soluciones conjuntas:</h5>
        <p>
            Inicia el proceso de certificación de las soluciones conjuntas (en aquellas que sea posible) para poder vender a través de canales alternativos, como los Marketplaces de la nube. Con el fin de que pases a la siguiente fase, te recomendamos seguir especializándote, y convertirte en un proveedor de soluciones certificado con amplios conocimientos técnicos. 
        </p>
        <h5>Soluciones: </h5>
        <p>
            Las soluciones pueden ser horizontales (tecnología) o verticales (orientadas al sector), pero también deben contar con una estrategia sólida de lanzamiento al mercado y de ventas asociada a cada una. A medida que vayas avanzando, deberás especializarse en la venta según las líneas de negocio, algo que podrás lograr comprometiéndote a la transformación del negocio y vendiendo soluciones tecnológicas integrales.
        </p>
        ',
        'stage5'=>'
        <p>
            Según la evaluaciónde IDC, tu organización se encuentra en la fase 5 respecto a tu mix de negocio en la nube. El 1% de las organizaciones que han participado en nuestro estudio se encuentran en esta fase. La mayoría de ellas están en la fase 3. Por lo general, las organizaciones que se encuentran en esta fase de desarrollo comparten las siguientes características:
        </p>
        <h3>Contribución a los ingresos: </h3>
        <p>
            En esta fase de desarrollo, los ingresos proceden principalmente de los servicios con una contribución muy sustanciosa a los ingresos recurrentes (a través del modelo SaaS y de los servicios gestionados). Las empresas tienen la suficiente solidez para prever sus negocios, lo que les permite una inversión y una planificación estratégicas.
        </p>
        <h3>Programas para Partners:</h3>
        <p>
            Normalmente, las organizaciones son capaces de dar soporte a varios entornos y productos de proveedores, y han alcanzado los niveles más altos de los Programas para Partners de proveedores de servicios en la nube. También cuentan con planes estructurados de formación con proveedores estratégicos, los cuales lesconsideran Partners de confianza.
        </p>
        <h3>Servicios:</h3>
        <p>
            Con una cartera completa de servicios, en esta fase de desarrollo las empresas ya han invertido en herramientas y tecnologías para permitir el soporte omnicanal (web, chat, correo electrónico, teléfono) en todas las interacciones con el cliente, y totalmente integrado con su CRM. La eficienciaen la entrega y la rentabilidad se supervisan detenidamente con KPIs y métricas de mejora.
        </p>
        <h3>Infraestructura en la nube:</h3>
        <p>
            En esta fase, las empresas normalmente han invertido, y siguen invirtiendo, en una infraestructura en la nube avanzada, muy automatizada y lógica, así como en herramientas y en tecnología de gestión. Por lo general, esto ofrece varias opciones para los clientes (on-premise, o en sus propias instalaciones, off-premise, o fuera de ellas, o en varios proveedores de servicios en la nube, o CSPs por sus siglas en inglés).
        </p>
        <h3>Soluciones: </h3>
        <p>
            En esta fase de desarrollo, se hace hincapié en la venta de soluciones conjuntas, así como de soluciones integrales con alto nivel de propiedad intelectual. Las soluciones forman parte de una agenda de transformación digital más amplia. Además, es importante ser capaz de desarrollar y gestionar soluciones integrales. Llegados a este punto, la escalabilidad se convierte en un KPI clave y es aconsejable que los Partners tengan la certificación de las soluciones de los proveedores, así como considerar la posibilidad de certificar las soluciones con propiedad intelectual para venderlas a través de los Marketplaces en la nube.
        </p>
        <!--<img src="http://sage.idcready.net/images/tools/1/business-mix.jpg" class="right">-->
        <h3>Recomendaciones</h3>
        <h5>Transformación digital:</h5>
        <p>
            En esta fase de tu mix de negocio, debes seguir invirtiendo en servicios y herramientas adaptadas a la transformación digital, entre los que se incluye la reestructuración de procesos y la integración directa de sistemas.
        </p>
        <h5>Inversión en soluciones de la Tercera Plataforma:</h5>
        <p>
            No dejes de invertir en las soluciones integradas de última generación, y combina las tecnologías digitales de la Tercera Plataforma, cuyos pilares son la movilidad, la nube, el análisis de Big Data, las redes sociales y los servicios asociados.
        </p>
        <h5>Impulsar la escalabilidad:</h5>
        <p>
            Evalúa las rutas hacia modelos de mercado y de negocio que permitan una escalabilidad eficaz, considera la posibilidad de tener otros Partners que puedan vender las licencias de las soluciones y amplía tu red de Partners para expandirte geográficamente.
        </p>
        ',
    ],
];
