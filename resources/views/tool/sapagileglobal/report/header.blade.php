...
<head>
	...
	<link rel="stylesheet" href="{{ asset('bundle/'.$manifest['sapagileglobal'][0])}}">
	<script type="text/javascript">
		...
		function subst() {
			...
			document.getElementsByClassName('swopimage')[0].style.backgroundImage = 'url({{ asset('images/tools/17/') }}/'+ string_to_slug(vars['section']) +'_banner.png)';
			document.getElementsByClassName('swopimage')[0].classList.remove('bg-transparent','bg-sap-green','bg-sap-purple','bg-grey','bg-sap-orange');
			document.getElementsByClassName('swopimage')[0].classList.add(colors[string_to_slug(vars['section'])]);
			...
			//document.getElementsByClassName('swopimage')[0].innerText = 'url({{ asset('images/tools/17/') }}/'+ string_to_slug(vars['section']) +'_banner.jpg)';
		}
	</script>
</head>
<body onload="subst()">
<div class="swopimage w-full text-white bg-center bg-cover bg-no-repeat overflow-hidden bg-blackblack" style="height: {{ App::environment('local') ? '21mm' : '27mm'}};">
	<div class="w-5/6 mx-auto">
		<h1 class="section font-thin uppercase text-{{ App::environment('local') ? '' : '3'}}xl pt-6">Click to be Agile</h1>
